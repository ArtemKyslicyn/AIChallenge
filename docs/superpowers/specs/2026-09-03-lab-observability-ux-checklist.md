...
Закрыты три из четырёх — снова по живому стенду, не по исходникам:
...
Остаётся открытым: **повторный клик по уже нажатой оценке переотправляет её вместо отмены**
(`FeedbackStrip` + эндпоинт). Не трогали намеренно — правка живёт в чужих файлах.
