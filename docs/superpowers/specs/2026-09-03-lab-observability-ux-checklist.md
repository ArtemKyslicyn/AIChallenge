...
| hint_n_folded | Прогонов в окне: {n} |
...
| formula_summary | Score = качество (или успех, пока оценок мало) ÷ время_ответа ÷ cost. Нужен баланс качества, скорости и цены. |
...
- `col_quality` / `hint_quality` — колонка качества (спека `2026-09-03-quality-column-design.md`).
  Подсказка описывает и число, и его цену: процент — среднее судьи по модели, а в скобках,
  в `title` ячейки, — `judged_n`, число оценённых прогонов. Без него среднее из семи прогонов
  читается как установленный факт. Колонка рисуется только тогда, когда в окне есть хоть одна
  оценка: с пустым `JUDGE_MODEL` судить некому, и таблица обязана выглядеть ровно как раньше —
  колонка прочерков выглядела бы сломанной фичей, а не выключенной.
- `formula_summary` переписан 2026-09-03: Score перестал быть одной формулой. Пока оценок
  меньше `JUDGE_MIN_RUNS`, числитель — прежний `успех`, дальше — `качество`. Строка обязана
  называть обе ветки, иначе читатель, увидевший переставленную таблицу, не найдёт причины.
  Хвост про баланс качества, скорости и цены оставлен: он и был про эти три оси.
