...
## D14 Экспертная UI/UX-валидация (фаза C.0)

P0 Task 7 требует один проход по эвристикам Нильсена. Этого мало для фичи, которая добавляет
третью плавающую панель и новый контрол внутрь каждого пузыря ответа. Валидация усилена до
**отряда специализированных ревьюеров, работающих по живому стенду, а не по коду**.

### Стенд

Локально, ничего не деплоя: throwaway Postgres в контейнере на свободном порту со своими
креденшелами (никогда dev-база — её conftest делает TRUNCATE), API с `USE_FAKE_LLM=true`,
`RUN_TRACE_ENABLED=true`, `MEDIA_TOOLS_ENABLED=true`, Vite dev против него. `.env` не читаем.
Перед ревью наполнить стенд данными: несколько сообщений, пара 👍/👎 — пустые таблицы нечего ревьюить.

### Состав отряда (параллельно, read-only)

| Ревьюер | Что смотрит |
|---------|-------------|
| `ux-auditor` | сквозной аудит по эвристикам + гейт из чеклиста |
| `panel-reviewer` | float-dock: три панели, mutex, размеры, фокус, скролл внутри |
| `density-reviewer` | плотность таблиц Рейтинг/Оценки, отсутствие overlap, поведение длинных `model_id` |
| `navigation-reviewer` | вкладки, клавиатура, порядок фокуса, возврат фокуса на FAB |
| `card-reviewer` | футер ответа ассистента: бейдж модели + FeedbackStrip в одной строке |

### Правила для правок (обязательные)

1. **Сохранять вайб.** Существующие токены, акцент, плотность, тон RU-копирайта. Никакой
   новой палитры, шрифтов, теней «как в дашборде», никакого фиолетового AI-glow.
2. **Не ломать то, что работало.** Композер, отправка, стрим, media-карточка, режимы
   compare/lab — регрессия здесь дороже любой находки по новой панели.
3. **Чинить обязательно** всё, что блокирует: severity ≥3, любой промах по a11y (фокус,
   клавиатура, контраст), поломка на ~390px.
4. **Мелкую полировку** делать только если она локальна и не трогает общие компоненты.
   Спорное — записывать в чеклист как follow-up, а не «заодно переделать».
5. Новые строки RU-копирайта сначала добавляются в чеклист микрокопии, потом в код.

### Известные пробелы копирайта (закрыть в C.0)

- нет ключа для лейбла раскрытия формулы (в коде «Как считается Score» из design spec §4)
- нет строки ошибки для вкладки «Оценки» (переиспользована `err_ranking`)
- нет ключей для accessible-имён контрола окна и таблиц вкладок
