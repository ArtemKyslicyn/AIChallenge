...
> ⚠️ **Исправление плана (2026-09-03, найдено при исполнении Tasks 1–6).**
> Шаг 2 ниже предполагал, что стадия каскада уже доезжает до фронта. Она не доезжает:
> ни `message_end`, ни `GET /sessions/{id}/messages` её не несут. Значит Task 7 начинается
> с бэкенда — двумя маленькими добавлениями, симметрично тому, как в фазе B доехала оценка:
>
> - `MessageEndEvent` и его SSE-фрейм получают `cascade_stage` — для **живого** ответа;
> - `MessageResponse` получает `cascade_stage` — для ответа **после перезагрузки**,
>   значение берётся из `run_traces` тем же приёмом, что и `feedback` (один запрос на сессию,
>   не N+1).
>
> Оба поля строковые и никогда не `null`: `"off"` — законное значение, означающее «каскад
